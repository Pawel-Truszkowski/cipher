...
class TestManager:
    ...
    def test_encrypt_should_print_encrypted_message_when_running_correct(
        self, manager, mock_message_manager, mock_cipher_manager
    ):
        with patch("builtins.input", return_value="Hello World"):
            with patch("builtins.print") as mock_print:
                manager._encrypt()
                mock_cipher_manager.encrypt.assert_called_once_with(
                    "Hello World", "rot_13"
                )
                mock_message_manager.add_message.assert_called_once_with(
                    "Hello World", "Encrypted Message", "rot_13"
                )
                mock_print.assert_called_once_with(
                    "Encrypted message is: Encrypted Message"
                )

    def test_decrypt_should_print_decrypted_message_when_running_correct(
        self, manager, mock_message_manager, mock_cipher_manager
    ):
        with patch("builtins.input", return_value="message"):
            with patch("builtins.print") as mock_print:
                ...
                mock_message_manager.add_message.assert_called_once_with(
                    "message", "Decrypted Message", "rot_13"
                )
                mock_print.assert_called_once_with(
                    "Decrypted message is: Decrypted Message"
                )
